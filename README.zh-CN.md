...
<p align="center">
  <img src="assets/hero-zh.png" alt="cc-session-memory — 给 Claude Code 的会话记忆与使用统计" width="640" />
</p>
...
| 维度 | claude-mem | cc-session-memory |
|---|---|---|
...
| **平台** | macOS / Linux（文件锁用 fcntl，不支持 Windows） |
| **代码总量** | ~2400 行，一下午能整体读完 |
...
![cc-session-memory 架构：Stop hook、后台 worker、LLM 总结、markdown 存储](docs/images/architecture.png)
...
./memory_system/bin/setup.sh --global   # 交互式让你粘 key，key 只落在本机 chmod-600 的 config 里
claude
```

> ⚠️ **API key 别粘进 Claude Code 对话。** 你输入的每个字都会原样存进 CC 的 transcript（还会被本工具自己摘要一遍）。key 的唯一入口是 `setup.sh`——不进 git，不进对话。

进 Claude Code 后粘贴：

> setup.sh 已经带我的 key 跑过了。请按本 repo 根目录 `INSTALL.md` 第 3 节"标准化安装步骤"完成剩余安装（全局模式）：校验 4 个落地点；我用的 provider 是 `<openai / anthropic / deepseek / ollama / …>`，需要的话按 Provider 矩阵改 config 的 `endpoint` / `model` / `protocol`；最后跑一次冒烟测试。每完成一步打勾汇报，遇到意外停下来问我。

hook、config、`/sess` `/sessme` `/ccskill` `/ccmcp` 四个 skill 由 Claude Code 校验配齐。手动安装路径和完整 [Provider 矩阵](INSTALL.md#provider-矩阵)（OpenAI / Anthropic / DeepSeek / Ollama / …）见 [INSTALL.md](INSTALL.md)（[English](INSTALL.en.md)）。

以后想换 provider，对 Claude Code 说一句"把我的 cc-session-memory 配置换成 deepseek"就行。
...
- **API key 进不了 git** —— 存在 `~/.config/cc-session-memory/config.json`（chmod 600），`.gitignore` 还兜底拦截一切误入的 `config.json`。
...
```
cc-session-memory/
├── INSTALL.md · INSTALL.en.md       # 安装指南（中文 · EN，从这里开始）
├── memory_system/
...
⭐ 如果 cc-session-memory 帮你记住过什么，点个 star。
