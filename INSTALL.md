...
> 请按本 repo 根目录的 `INSTALL.md` 第 3 节"标准化安装步骤"帮我装 cc-session-memory。
...
> 如果不是 OpenAI，请按 INSTALL.md 的 Provider 矩阵帮我把 `~/.config/cc-session-memory/config.json`
...
> 把我的 `~/.config/cc-session-memory/config.json` 改成 deepseek。
