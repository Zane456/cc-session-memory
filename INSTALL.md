# 安装 cc-session-memory

[English](INSTALL.en.md) | 简体中文

整套系统装好需要 4 块东西：

1. **`~/.config/cc-session-memory/config.json`** — 你挑的 LLM provider（OpenAI / Anthropic / DeepSeek / 本地 Ollama / Z.AI 等任一）的 endpoint + model + api_key（**不在 repo，必须本机生成**）
2. **`~/.claude/settings.json` 的 Stop hook** — 让 Claude Code 每轮调本系统的 worker
3. **skill 装到 `~/.claude/skills/`** — `/sess`（会话检索）+ `/sessme`（只调本窗口当前 session，多窗口不串）+ `/ccskill` `/ccmcp`（skill / MCP 使用统计）
...
- macOS 或 Linux（文件锁用 `fcntl`，不支持 Windows）
...
./memory_system/bin/setup.sh --global    # 交互式让你粘 key；key 只写进本机 chmod-600 的 config
claude
```

> ⚠️ **API key 不要粘进 Claude Code 对话。** 对话原文会一字不差存进 `~/.claude/projects/` 的 transcript（还会被本工具自己摘要进 memories/）。key 的正确入口只有 `setup.sh`：交互输入不留 shell 历史；脚本场景可用 `--key <key>` 或 `ZAI_API_KEY` 环境变量。

进 Claude Code 后，把下面这段一字不漏粘进去：

> setup.sh 已经带我的 key 跑过了。请按本 repo `INSTALL.md` 第 3 节"标准化安装步骤"完成剩余安装（全局模式）：
> 校验 4 个落地点。我用的 LLM provider 是 `<openai / anthropic / deepseek / openrouter / ollama / z.ai 等>`，
> 如果不是 OpenAI，请按 Provider 矩阵帮我把 `~/.config/cc-session-memory/config.json`
> 的 `endpoint` / `model` / `protocol` 三个字段对应改掉（不用动 api_key）。
> 最后跑一次冒烟测试，确认 hook 触发、memories md 生成、`/sess` skill 加载正常。
> 每完成一步给我打勾汇报，遇到任何意外停下来问我。

Claude 会校验落地点、按你说的 provider 调整 config、跑一次端到端冒烟。任何意外它会停下来问你。
...
setup.sh 默认把 config 指向 OpenAI Chat Completions（`gpt-4o-mini`）。要换 provider，跑完之后编辑 `~/.config/cc-session-memory/config.json` 的 `endpoint` / `model` / `protocol` 三个字段——见下面的 [Provider 矩阵](#provider-矩阵)。

`setup.sh --global` 一次完成：

| 步骤 | 写到哪 | 备注 |
|---|---|---|
| 创建 config | `~/.config/cc-session-memory/config.json` (chmod 600) | 已有则保留，加 `--force` 覆盖（先备份 .bak）|
...
uname -s                # 需要 Darwin 或 Linux（fcntl 锁，不支持 Windows）
```

### 2. 跑 setup.sh（用户若已自己跑过，跳到第 3 步）
...
如果调用失败（API key 错、endpoint 不通、model 名拼错等），错误会落到 `~/.config/cc-session-memory/failures/`，不会污染 memories/。
...
> 给我讲个笑话                          # 随便聊一轮
> /exit
ls -lt ~/.config/cc-session-memory/logs/        # 应当有新的 run-*.log 和 worker.log 增长
...
Claude Code 自己每轮把完整 transcript 写在 `~/.claude/projects/<encoded-cwd>/<session-uuid>.jsonl`，长期会膨胀（cc-session-memory 的 GLM 摘要不依赖它，但 `/sess --raw` 模式依赖；删掉旧的就只能看摘要）。
...
> 帮我用 launchd 配一个每天 22:00 自动跑 `<repo>/memory_system/bin/prune_cc_transcripts.py` 的 plist，标签叫 `com.cc-session-memory.prune-transcripts`。
...
cc-session-memory 调 LLM 走 **OpenAI Chat Completions** 或 **Anthropic Messages** 协议——基本所有现代 LLM 提供商都至少支持其中一种。挑一个填到 `~/.config/cc-session-memory/config.json`：
...
如果新 provider 要换 key，先在**对话外**把 key 喂进去（和安装时同一条规矩）：

```bash
./memory_system/bin/setup.sh --force --key <新-provider-key>   # 其余 config 字段保留，旧文件备份 .bak
```

然后在 Claude 里说：

> 把我的 `~/.config/cc-session-memory/config.json` 改成 deepseek——只改 endpoint / model / protocol 三个字段，key 已经就位。
