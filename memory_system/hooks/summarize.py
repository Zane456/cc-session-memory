...
每次 Claude Code 完成一轮回答 → Stop hook 触发 → 这个 worker 跑一次：
  ...
  2. 检查 last_assistant_message：太短（< 50 chars）→ 跳过，不调 GLM
  3. 从 transcript JSONL 抽出"最后一对" (user prompt, assistant final text, tool 文件清单)
  4. 调 GLM 总结这一轮（≤ 100 字，max_tokens=200）
  5. 文件操作（带 fcntl 排它锁）：
...
DEFAULTS = {
    ...
    "model": "glm-5-turbo",
    "temperature": 0.3,
    "max_tokens": 200,           # 每轮一句两句的极简总结，200 就够
    "request_timeout_seconds": 60,
    "max_db_size_mb": 200,
    "min_assistant_chars": 50,   # 这一轮 assistant 内容低于这个长度就跳过 GLM
...
PROMPT_TURN_ZH = """请用 2-3 句中文简要总结下面这一轮 Claude Code 对话，并给出关键词。

严格按这个格式输出（不带 frontmatter，不带 ## 标题，每行就是字面这几行）：

用户: <1 句话总结用户的核心意图，≤ 30 字>
结果: <1-2 句话总结 Claude 给的最终答案/产出，≤ 60 字>
关键词: <3-5 个关键词，逗号分隔>
...
def call_glm_for_turn(
    cfg: dict[str, Any],
    user_text: str,
    assistant_text: str,
) -> dict[str, Any]:
    """调 GLM 总结单轮，返回 {summary, model, usage, elapsed}。
    summary 是 GLM 直出的 3 行文本，不含 frontmatter。"""
    user_msg = PROMPT_TURN_ZH.format(user_text=user_text, assistant_text=assistant_text)

    payload = {
        "model": cfg["model"],
        "system": "你是单轮对话总结助手。输出短、准、可 grep。",
    ...
    log.info("calling GLM (turn): model=%s", cfg["model"])
    started = time.time()
    try:
        ...
        raise RuntimeError(f"GLM HTTP {e.code}: {err_body[:500]}") from e
    elapsed = time.time() - started

    data = json.loads(raw)
    if "content" not in data or not data["content"]:
        raise RuntimeError(f"unexpected GLM response: {raw[:500]}")
    summary = data["content"][0].get("text", "").strip()
    usage = data.get("usage") or {}
    log.info("GLM ok in %.2fs · usage=%s", elapsed, usage)
    return {"summary": summary, "model": data.get("model", cfg["model"]), "usage": usage, "elapsed": elapsed}
...
def parse_turn_summary(s: str) -> dict[str, str]:
    """把 GLM 输出的"用户:.../结果:.../关键词:..." 三行解析成 dict。
    GLM 偶尔会乱加 markdown 强调，宽松匹配。"""
...
def _write_failure_note(cfg: dict[str, Any], event: dict[str, Any], err: str) -> None:
    """GLM 失败写到 ~/.config/cc-memory/failures/，不污染 memories/（不会被推到 GitHub）。"""
    try:
...
def main(payload_file: str) -> int:
    ...
    try:
        result = call_glm_for_turn(cfg, turn["user_text"], turn["assistant_text"])
    except Exception as e:
        log.error("GLM call failed: %s\n%s", e, traceback.format_exc())
        try:
